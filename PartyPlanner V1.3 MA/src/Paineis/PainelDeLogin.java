...
import Tela.Janela;
import Tratamentos.EmailInvalidoException;
import Tratamentos.SenhaInvalidaException;
	...
	public void actionPerformed(ActionEvent Clique) {
		...
		switch (play) {
			...
			case "Entrar":
                ...
                try {
                	for(Usuario user: central.getTodosOsAdministradores()) {
                	SenhaInvalidaException erroSenha;
                	EmailInvalidoException erroEmail;
                	if(user.getEmail().equals(email)) {	
						if(user.getSenha().equals(senha)) {	
							Janela.setPanel(new PainelDoAdm((Administrador)user));
							break;
						}
						else {
							throw erroSenha = new SenhaInvalidaException();
						}
                	}
                	else {
					throw erroEmail = new EmailInvalidoException(email);
				}
					
                }
                }catch(EmailInvalidoException erro) {
					JOptionPane.showMessageDialog(null, erro.getErro(),"Email incorreto ",JOptionPane.WARNING_MESSAGE);
					break;
                }catch(SenhaInvalidaException erro) {
					JOptionPane.showMessageDialog(null, erro.getErro(),"Senha Incorreta ",JOptionPane.WARNING_MESSAGE);
					break;
                }
