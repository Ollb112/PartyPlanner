			...
			case ("Cadastrar"):
				...
				for (int i = 0; i < cdi.getTodosOsFornecedores().size(); i++) {
					if (i == opcoes.getSelectedIndex()) {
						try {
							if(!campoTipoDeServico.getText().equals(null)) {
								if(!campoDescricao.getText().equals(null)) {
									...
									CamposNulosException erro = new CamposNulosException("Campo de descrição");
									throw erro;
									}
								}
							else {
								CamposNulosException erro = new CamposNulosException(" Tipo de serviços");
								throw erro;
								
							}
						}
						catch(CamposNulosException erro) {
							JOptionPane.showMessageDialog(null, erro.getErro(),"Campo Invalido ",JOptionPane.WARNING_MESSAGE);
							break;
